...
from ptodo import (__app_name__, __version__,
                   DB_READ_ERROR, cli, ptodo, SUCCESS)
...
def mock_json_file(tmp_path):
    todo = [{"description": "test01", "priority": 2, "done": False},]
...
def test_add(mock_json_file, description, priority, expected):
    todoer = ptodo.Todoer(mock_json_file)
